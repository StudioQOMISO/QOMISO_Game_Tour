...
const files={active:"rider_parameters_active_300.csv",retired:"rider_parameters_retired.csv",pending:"rider_parameters_status_pending.csv"};
if(!files[group])throw new Error(`unknown group ${group}`);
const src=parseCsv(await fs.readFile(`data/${files[group]}`,"utf8"));
